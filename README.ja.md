...
検査は最後の 1 回も含めて毎回走る。`maxRetries: 0` は「1 回だけ実行して、それも検査する」という意味に
なる。通らなかった回答が完了することはない — 再試行を使い切ってもまだ不合格なら、違反の内訳と却下した
本文を添えてタスクを失敗させる。検証済みの回答と取り違えようがない形で返す。

**「幻覚を防ぐ」と言う前に、限界が 1 つ。** 検証するのは出どころだけで、裏づけではない。fetch 済みの
ページは、そこに書かれていない主張、矛盾する主張にでも付けられる。排除できるのは「このタスクで fetch
していない URL の引用」— 学習データから思い出した URL、でっち上げた URL、スニペットで見ただけで開いて
いない URL である。
